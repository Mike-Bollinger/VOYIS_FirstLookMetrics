    ...
    def export_visibility_metrics(self, output_path: str, analysis_results: Dict) -> None:
        ...
        try:
            ...
            with open(metrics_file, 'w') as f:
                ...
                # Basic statistics
                f.write("VISIBILITY ANALYSIS STATISTICS\n")
                f.write("-" * 32 + "\n")
                f.write(f"Total Images Found: {analysis_results.get('total_images_found', 0)}\n")
                f.write(f"Images Analyzed: {analysis_results.get('images_analyzed', 0)}\n")
                f.write(f"Analysis Errors: {analysis_results.get('errors', 0)}\n")
                ...
                if by_category:
                    ...
                    f.write("-" * 32 + "\n")
                    total_analyzed = sum(by_category.values())
                    
                    for category, count in by_category.items():
                        percentage = (count / total_analyzed * 100) if total_analyzed > 0 else 0
                        category_display = category.replace('_', ' ').title()
                        f.write(f"{category_display}: {count} images ({percentage:.1f}%)\n")
                    ...
                    if good_percentage >= 80:
                        f.write("Overall Visibility Quality: EXCELLENT (≥80% good/great)\n")
                    elif good_percentage >= 60:
                        f.write("Overall Visibility Quality: GOOD (60-79% good/great)\n")
                    elif good_percentage >= 40:
                        f.write("Overall Visibility Quality: FAIR (40-59% good/great)\n")
                    else:
                        f.write("Overall Visibility Quality: POOR (<40% good/great)\n")
                    
                    f.write(f"Good/Great Visibility: {good_count} images ({good_percentage:.1f}%)\n")
                    f.write(f"Poor/Zero Visibility: {poor_count} images ({poor_percentage:.1f}%)\n")
                else:
                ...
                if csv_path:
                    f.write(f"\nOUTPUT FILES\n")
                    f.write("-" * 32 + "\n")
                    f.write(f"Results CSV: {os.path.basename(csv_path)}\n")
                    f.write(f"Chart: Image_Visibility_Chart.png\n")
                
                f.write("\n" + "=" * 55 + "\n")
                f.write("Generated by VOYIS First Look Metrics\n")
                
            print(f"Visibility metrics exported to: {metrics_file}")
